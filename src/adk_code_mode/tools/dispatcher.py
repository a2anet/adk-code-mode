...
def _raise_for_failed_result(tool: BaseTool, result: Any) -> None:
    """Raise for a failure a tool returned as a value.

    ADK hands these back as ordinary results because in a plain agent the model
    reads them. Sandbox code reads return values instead, so a loop would carry on
    as if every call had worked.
    """
    if not isinstance(result, dict):
        return
    inner = tool._wrapped if isinstance(tool, ToolResultArtifactTool) else tool
    if isinstance(inner, RestApiTool) and set(result) == {"error"}:
        ...
        *,
        ...
        if response is None:
            try:
                ...
                _raise_for_failed_result(tool, response)
